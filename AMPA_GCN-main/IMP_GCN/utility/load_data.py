...
class Data(object):
    def __init__(self, path, batch_size):
        ...
        self.R = sp.dok_matrix((self.n_users, self.n_items),
                               dtype=np.float32)
    ...
    def generateKorderGraph_UUII(self, iiK):
        u_batch_size = 100
        checkpoint_path = "weights/gowalla/AMPA_GCN_pre_AMPA_GCN_l6/64-64-64-64-64-64/l0.001_r0.0001/weights-429"
        NewCheck = tf.compat.v1.train.NewCheckpointReader(checkpoint_path)

        item_train_emb = NewCheck.get_tensor("item_embedding")
        ...
        ego_embeddings = item_train_emb
        ii_pred = tf.nn.sigmoid(tf.matmul(ego_embeddings, tf.transpose(ego_embeddings)))


        with tf.compat.v1.Session() as sess1:


            ii_pred_value = sess1.run(ii_pred)
            ii_pred = np.copy(ii_pred_value)
        ...
        for batch_i in range(ii_partial_batch_pred_list.shape[0]):
            iid = items[batch_i]
        ...
        return [distill_ii_row, distill_ii_col, distill_ii_value]



    def reset_graph_uuii(self, newdata):

        new_ii_row, new_ii_col, new_ii_val = newdata
        ...
        # 创建稀疏矩阵
        self.ItemItemNet = csr_matrix((new_ii_val, (new_ii_row, new_ii_col)), shape=(self.n_items, self.n_items))
    ...
    def create_adj_mat(self):
        ...
        i_i
        """
        ItemItemN = self.ItemItemNet.tolil()
        ...
        for i in range(5):
            ...
            adj_mat[(int(self.n_users)+int(self.n_items * i / 5.0)):(int(self.n_users)+int(self.n_items * (i + 1.0) / 5)),self.n_users:] = \
                ItemItemN[int(self.n_items * i / 5.0):int(self.n_items * (i + 1.0) / 5)]
