...
class AMPA_GCN(object):

    def __init__(self, data_config, pretrain_data):
        # argument settings
        self.model_type = 'AMPA_GCN'
        ...
        '''
        *********************************************************
        Create Placeholder for Input Data & Dropout. 为输入数据和删除创建占位符
        '''
        ...
        *********************************************************
        ...
        self.ua_embeddings, self.ia_embeddings, self.A_fold_hat_group_filter, self.user_group_embeddings = self._create_ampa_gcn_embed()
        """
        *********************************************************
    ...
    def init_weights(self):
        ...
        # W3
        all_weights['W_gc_3'] = tf.Variable(initializer([self.emb_dim, self.emb_dim]), name='W_gc_3')
        all_weights['b_gc_3'] = tf.Variable(initializer([1, self.emb_dim]), name='b_gc_3')
        # W4
        all_weights['W_gc_4'] = tf.Variable(initializer([self.emb_dim, self.emb_dim]), name='W_gc_4')
        all_weights['b_gc_4'] = tf.Variable(initializer([1, self.emb_dim]), name='b_gc_4')
    ...
    def _create_ampa_gcn_embed(self):
        ...
        self_embeddings = tf.matmul(ego_embeddings, self.weights['W_gc_1']) + self.weights['b_gc_1']
        self_embeddings2 = tf.matmul(tf.concat(temp_embed, 0), self.weights['W_gc_3']) + self.weights['b_gc_3']
        bi_embeddings=tf.multiply(tf.concat(temp_embed, 0),ego_embeddings)
        bi_user_group_embeddings_hidden_1 = tf.matmul(bi_embeddings, self.weights['W_gc_2']) + self.weights['b_gc_2']


        sum_embeddings = self_embeddings+bi_user_group_embeddings_hidden_1+self_embeddings2
        user_group_embeddings_hidden_1=tf.nn.leaky_relu(sum_embeddings)





        user_group_embeddings_hidden_2 = tf.nn.leaky_relu(tf.matmul(user_group_embeddings_hidden_1, self.weights['W_gc_4']) + self.weights[
           'b_gc_4'])
        ...
        u_group_embeddings = tf.ones(tf.shape(u_group_embeddings))
...
if __name__ == '__main__':
    ...
    i_i
    """
    [distill_ii_row, distill_ii_col, distill_ii_value] = data_generator.generateKorderGraph_UUII(iiK=5)
    data_generator.reset_graph_uuii([distill_ii_row, distill_ii_col, distill_ii_value])
    ...
    model = AMPA_GCN(data_config=config, pretrain_data=pretrain_data)
    ...
    if args.save_flag == 1:
        ...
        weights_save_path = '%sweights/%s/%s/%s/l%s_r%s' % (args.weights_path,
                                                            args.dataset,
                                                            model.model_type,
                                                            layer,
                                                            str(args.lr),
